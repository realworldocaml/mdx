...
Here we define a package test that depends on a package `a`:
  ...
  $ opam show --just-file --no-lint --raw -fdepends ./existing.opam
  dune, a
...
exist in our repo:
  ...
  $ opam show --just-file --no-lint --raw -fdepends ./toonew.opam
  "dune" "a" {>= "1.0"}
...
We should also produce the right error message with all the constraints when we have multiple constaints
  ...
  $ opam show --just-file --no-lint --raw -fdepends ./multiple-constraint.opam
