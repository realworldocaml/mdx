...
(** Tests that the result of 'cd <dir_name> && ocaml-mdx format [options] test-case.md'
...
let pp_expect_action fmt dir =
  ...
  (with-stdout-to %%{target}
   (chdir %s
    (run ocaml-mdx format --output - %a%s)))|}
    dir.dir_name pp_options dir.options dir.test_file

(** Tests that 'cd <dir> && ocaml-mdx format [options] <file>' exits with a
