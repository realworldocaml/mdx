...
We have a file that depends on package "b".
  ...
  $ opam show --just-file --no-lint --raw -fdepends ./vendored.opam
  dune, b

This package does not set any [opam]-provided packages:

  $ opam show --just-file --no-lint --raw -fx-opam-monorepo-opam-provided ./vendored.opam
...
`opam-monorepo` will vendor it.
  ...
  $ opam show --just-file --no-lint --raw -fdepends ./vendored.opam.locked
  "b" {= "1" & ?vendor}
  "base-bigarray" {= "base"}
  "base-threads" {= "base"}
  "base-unix" {= "base"}
...
Asking for the value will return that "b" will be provided by Opam:
  ...
  $ opam show --just-file --no-lint --raw -fx-opam-monorepo-opam-provided ./opam-provided.opam
  b
...
`opam` on it, it will install the package "b".
  ...
  $ opam show --just-file --no-lint --raw -fdepends ./opam-provided.opam.locked
  "b" {= "1"}
  "base-bigarray" {= "base"}
  "base-threads" {= "base"}
  "base-unix" {= "base"}
...
depend transitively on an `opam`-provided package.
  ...
  $ opam show --just-file --no-lint --raw -fdepends ./transitive.opam
  dune, depends-on-b
  $ opam show --just-file --no-lint --raw -fx-opam-monorepo-opam-provided ./transitive.opam
  b

Locking it should work as usual

  $ opam-monorepo lock transitive > /dev/null
  $ opam show --just-file --no-lint --raw -fdepends ./transitive.opam.locked
  "b" {= "1"}
  "base-bigarray" {= "base"}
  "base-threads" {= "base"}
  "base-unix" {= "base"}
...
Now for the reverse case, `depends-on-b` is `opam`-provided.
  ...
  $ opam show --just-file --no-lint --raw -fdepends ./reverse-transitive.opam
  dune, depends-on-b
  $ opam show --just-file --no-lint --raw -fx-opam-monorepo-opam-provided ./reverse-transitive.opam
  depends-on-b

Since it is, we need to make its dependency, "b", also `opam`-provided.

  $ opam-monorepo lock reverse-transitive > /dev/null
  $ opam show --just-file --no-lint --raw -fdepends ./reverse-transitive.opam.locked
  "b" {= "1"}
  "base-bigarray" {= "base"}
  "base-threads" {= "base"}
  "base-unix" {= "base"}
...
be ignored is not a string.
  ...
  $ opam show --just-file --no-lint --raw -fx-opam-monorepo-opam-provided ./warning.opam
  42
...
not:
  ...
  $ opam show --just-file --no-lint --raw -fx-opam-monorepo-opam-provided ./warning-list.opam
  42 "fourtytwo"
  $ opam-monorepo lock warning > /dev/null
  opam-monorepo: [ERROR] Error in opam file $TESTCASE_ROOT/warning.opam, [8:31]-[8:33]: Expected a string or a list of strings, got: 42
  [1]
