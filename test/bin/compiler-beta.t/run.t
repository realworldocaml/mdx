We have a simple project that only depends on OCaml
  ...
  $ opam show --just-file --no-lint -fdepends ./test.opam
  ocaml, dune
...
avoid-version flag:
  ...
  $ opam show --just-file --no-lint -fflags repo-with-beta-ocaml/packages/ocaml-base-compiler/ocaml-base-compiler.4.14.0~beta1/opam
...
should pick 4.13.1:
  ...
  $ opam show --just-file --no-lint -fdepends ./test.opam.locked | grep ocaml
  "ocaml" {= "4.13.1"}
...
Now if we require ocaml >= 4.14, as the following package does:
  ...
  $ opam show --just-file --no-lint -fdepends ./test-requires-beta.opam
  "ocaml" {>= "4.14"} "dune"
...
satisfying version available:
  ...
  $ opam show --just-file --no-lint -fdepends ./test-requires-beta.opam.locked | grep ocaml
  "ocaml" {= "4.14.0"}
