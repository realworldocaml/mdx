...
let pos_errorf ~pos fmt =
  ...
  Format.ksprintf (fun msg -> Error (`Msg msg))
...
let rec to_sexp_strict value =
  ...
  | List (_, l) ->
    Result.List.all (List.map ~f:to_sexp_strict l) >>| fun l ->
  ...
  | Group (pos, _)
  | Option (pos, _, _)
  | Env_binding (pos, _, _, _) -> pos_errorf ~pos "Expected a sexp compatible opam value"
